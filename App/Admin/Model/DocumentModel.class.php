...
class DocumentModel extends Model{
    ...
    public function detail($id){
        ...
        if($logic){
        	...
        	//$this->error='对应的逻辑处理类不存在则直接查询数据';
        	$modelname="kl_document_".ucwords(get_document_model($info['model_id'], 'name'));
        	$data=$this->where("kl_document.id=$id")
        	           ->join("$modelname as a on a.id=kl_document.id")->find();  
        	return $data;            
        }
...
//     	}
    ...
    private function logic($model){
      //var_dump(get_document_model($model, 'name'));
      $model=D(get_document_model($model, 'name'), 'Logic');
      $str=get_class($model);
       if($str!='Think\Model'){
       	return $model;
       }else{
